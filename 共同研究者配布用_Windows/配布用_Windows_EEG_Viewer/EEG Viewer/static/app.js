...
const DEFAULT_MULTI_MONTAGES = ["longitudinal", "a1a2", "average", "transverse"];
...
const RESEARCH_RATINGS = ["IEDs present", "IEDs absent", "判定困難"];
...
const els = {
  ...
  researchExportCsvBtn: document.getElementById("researchExportCsvBtn"),
...
function bindResearchControls() {
  ...
  els.researchExportCsvBtn?.addEventListener("click", exportResearchCsv);
  els.researchCompleteExportCsvBtn?.addEventListener("click", exportResearchCsv);
...
function researchCsvFilename(readerId, profile = researchProfile()) {
  const name = safeCsvFilenamePart(profile.readerName || profile.doctorName || readerId, "reader");
  const affiliation = safeCsvFilenamePart(profile.affiliation || "", "");
  const id = safeCsvFilenamePart(readerId || profile.email || "reader", "reader");
  return [name, affiliation, id, "responses"].filter(Boolean).join("_") + ".csv";
}
...
function updateResearchControlsVisibility() {
  ...
  for (const section of document.querySelectorAll("[data-research-section]")) {
    ...
    else if (key === "dataset") section.hidden = !datasetMode;
    else if (key === "cut") section.hidden = !datasetMode;
    else if (key === "test") section.hidden = datasetMode;
    else if (key === "output") section.hidden = datasetMode;
  }
  setResearchControlVisible(els.researchCreateDatasetBtn, false);
  setResearchControlVisible(els.researchLoadDatasetBtn, false);
  setResearchControlVisible(els.researchDatasetPathInput, datasetMode);
  setResearchControlVisible(els.researchIedsPresentPathInput, !datasetMode);
  setResearchControlVisible(els.researchIedsAbsentPathInput, !datasetMode);
  if (els.researchDatasetPathInput) {
    els.researchDatasetPathInput.placeholder = "output folder";
    els.researchDatasetPathInput.title = "Output folder for cut EDF epochs";
  }
...
function showResearchCompletion() {
  ...
  const outputPath = researchProfile().outputPath;
  ...
  if (els.researchCompleteMessage) {
    els.researchCompleteMessage.textContent = outputPath
      ? `CSV output folder: ${outputPath}`
      : "CSV output folderが空の場合は、ダウンロードとしてCSVを保存します。";
  }
}
...
function setResearchMode(mode) {
  state.researchMode = mode === "test" ? "test" : "dataset";
...
function activeResearchCases() {
  if (state.researchMode === "test" && state.researchSession) return state.researchSession.cases || [];
...
function renderResearchPanel() {
  ...
  if (!dataset) {
    els.researchPanel.innerHTML = state.researchMode === "dataset"
      ...
      : '<div class="research-empty">No dataset loaded.</div>';
  ...
  const answered = session ? `${session.answeredCount || 0}/${session.totalCount || cases.length}` : `${cases.filter((row) => row.include !== false).length}/${cases.length} included · ${sampleCount}/group`;
  const modeLabel = state.researchMode === "test" ? `Phase ${escapeHtml(session?.phase || "")}` : "Cut";
  ...
  `;
  if (els.researchCaseEditor) els.researchCaseEditor.hidden = !current || state.researchMode === "test";
  if (current && state.researchMode !== "test") {
...
function syncResearchDesignControls(settings) {
  ...
  if (els.researchPhase2SampleToggle) els.researchPhase2SampleToggle.checked = settings.phase2SampleEnabled !== false;
  if (els.researchFalsePositiveToggle) els.researchFalsePositiveToggle.checked = settings.phase2FalsePositiveEnabled !== false;
}
...
function researchDesignSettings() {
  ...
  return {
    ...
    phase2SampleEnabled: els.researchPhase2SampleToggle ? els.researchPhase2SampleToggle.checked !== false : false,
    phase2FalsePositiveEnabled: els.researchFalsePositiveToggle ? els.researchFalsePositiveToggle.checked !== false : false,
  };
}
...
function renderResearchProgress() {
  ...
  els.researchTestProgress.innerHTML = `
    <div class="research-progress-card">
      <div class="research-progress-head"><strong>Phase ${escapeHtml(session.phase || "")}</strong><span>Progress ${pct}%</span></div>
      <div>${answered}/${total} answered · 残り ${remaining} 問</div>
      <div class="research-progress-bar"><span style="width:${Math.max(0, Math.min(100, pct))}%"></span></div>
      <div class="research-small">Showing ${displayIndex}/${cases.length || 0}${isResearchPracticeCase(current) ? " · 説明用テスト問題" : (current?.sampleEpoch ? " · sample epoch" : "")}</div>
    </div>
  `;
  renderRightResearchPanels();
}
...
function researchCaseLabelGroup(caseRow) {
  ...
  if (group === "epileptiform") return "IEDs present";
  if (group === "non_epileptiform") return "IEDs absent";
  return group || "-";
}
...
function researchResponseCorrectness(response = state.lastResearchResponse) {
  ...
  const rating = String(response.rating || "");
  if (!expected || rating === "判定困難" || rating === "uncertain" || !rating) return { label: "判定困難", className: "unknown", expected };
...
function researchRatingLabel(rating) {
  return rating === "uncertain" ? "判定困難" : (rating || "");
}
...
function renderRightTestPanel() {
  ...
  const currentRows = current ? [
    ...
    ["Event", formatSec(Number(current.eventTime ?? current.topomapTime ?? current.epochStart ?? 0))],
...
async function createResearchDataset() {
  ...
  const phase1Montage = storedResearchProfile().usualMontage || activeMontageValue();
...
async function showResearchCase(index) {
  ...
  if (!cases.length) {
    ...
    setStatus("Test complete. CSV export is ready");
    return;
  }
  state.researchCaseIndex = Math.max(0, Math.min(cases.length - 1, index));
  const item = cases[state.researchCaseIndex];
  renderResearchPanel();
  setStatus(`${isResearchPracticeCase(item) ? "Loading explanation practice epoch" : (item.sampleEpoch ? `Loading ${state.researchSession?.phase === "2" ? "Phase 2" : "Phase 1"} sample` : `Loading test epoch ${state.researchCaseIndex + 1}/${cases.length}`)}...`, { busy: true });
  try {
    ...
    state.cursorTime = Number(item.topomapTime ?? item.eventTime ?? item.epochStart ?? 0);
    state.rangeStart = null;
    state.scalogramSelection = null;
    state.scalogramData = null;
    const profile = researchProfile();
    const usualMontage = profile.usualMontage || item.phase1Montage || "a1a2";
    ...
    const remaining = Math.max(0, total - answered);
    setStatus(isResearchPracticeCase(item) ? "説明用テスト問題: 普段使うmontageに合わせて三択を選んでください" : (item.sampleEpoch ? `Phase ${state.researchSession?.phase || ""} sample` : `Test ${state.researchSession?.phase || ""}: ${state.researchCaseIndex + 1}/${cases.length} · 残り ${remaining} 問`));
    renderRightResearchPanels();
  } catch (err) {
  ...
  try {
    if (item.sampleEpoch) {
      if (isResearchPracticeCase(item)) {
        ...
        showResearchToast(`普段使うmontage: ${MONTAGE_LABELS[usualMontage] || usualMontage} · テスト開始`);
      } else {
        showResearchToast("Sample shown");
      }
      const cases = activeResearchCases();
      if (state.researchCaseIndex + 1 < cases.length) await showResearchCase(state.researchCaseIndex + 1);
      else {
        showResearchCompletion();
        setStatus("Test complete. CSV export is ready");
      }
    ...
    const data = await fetchJson("/api/research/test/response", {
      ...
      body: JSON.stringify({
        ...
        answeredAt,
        elapsedMs,
        displayMode: state.researchSession.phase === "2" ? "phase2_4montage_topomap" : "phase1_single",
    ...
    renderRightResearchPanels();
    showResearchToast(`Saved: ${rating} · やり直す場合はUndo`);
    const cases = activeResearchCases();
    if (cases.length) await showResearchCase(0);
    else if (state.researchSession.phase === "1" && state.researchDataset?.settings?.phase2FalsePositiveEnabled !== false) {
      setStatus("Phase 1 complete. CSV saved. Loading Phase 2...", { busy: true });
      const nextSession = await fetchJson(`/api/research/test/session?${qs({ dataset: state.researchDatasetPath, readerId: state.researchSession.readerId, phase: "2" })}`);
      state.researchSession = nextSession;
      setResearchResponsesFromSession(nextSession);
      state.researchCaseIndex = 0;
      renderResearchPanel();
      await showResearchCase(0);
    } else {
...
function showResearchToast(message) {
...
async function exportResearchCsv() {
  ...
  const readerId = researchReaderDisplayId(profile);
  try {
    const url = `/api/research/test/export.csv?${qs({ dataset: datasetPath, readerId, outputPath: profile.outputPath })}`;
    ...
    a.download = researchCsvFilename(readerId, profile);
    a.click();
    URL.revokeObjectURL(a.href);
    setStatus("Research CSV exported");
  } catch (err) {
...
function restoreSettings() {
  try {
    ...
    state.activeMontage = els.montageSelect.value || "longitudinal";
    state.viewMode = "single";
...
function activeMontageValue() {
  return els.montageSelect.value || state.activeMontage || "longitudinal";
}
...
function drawScalogramSelection(ctx, left, top, plotW, plotH, start, duration, ratio) {
  ...
  ctx.lineWidth = Math.max(0.5, 0.6 * ratio);
...
function drawCursorLine(ctx, left, top, plotW, plotH, start, duration, ratio) {
  ...
  ctx.lineWidth = Math.max(0.5, 0.55 * ratio);
  ctx.beginPath();
  ctx.moveTo(x, top);
  ctx.lineTo(x, top + plotH);
  ctx.stroke();
