...
LOG="${ROOT}/sync_open_eeg_viewer.log"
REPO_URL="git@github.com:saitosatoshi-1/eeg-viewer-montage-analysis.git"

exec > >(tee -a "${LOG}") 2>&1

echo "EEG Viewer sync, update, and open"
...
if [ -d "${ROOT}/.git" ]; then
  ...
  if git -C "${ROOT}" fetch --prune origin && git -C "${ROOT}" pull --ff-only --autostash origin main; then
    ...
    echo
    echo "WARNING: Could not pull latest changes from GitHub."
    echo "Make sure this Mac can access:"
    echo "${REPO_URL}"
    echo "Continuing with the files currently in this folder."
    echo
  fi
else
  echo "No .git directory found. Using the files currently in this folder."
  echo
fi
...
EEG_VIEWER_NO_PAUSE=1 /bin/bash "${UPDATE_SCRIPT}"

echo
echo "Opening EEG Viewer..."
